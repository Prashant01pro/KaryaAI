...
const TasksListPage = ({ type = 'all' }) => {
    ...
    const filteredTasks = tasks.filter(task => {
        ...
        if (type === 'ai-assistant') {
            return task.aiStrategy && matchesSearch;
        }
                ...
                <div className="space-y-6">
                    {loading && filteredTasks.length === 0 ? (
                        ...
                        <div className="text-center py-32 bg-surface-container-lowest rounded-[3rem] border border-dashed border-surface-variant/20">
                            ...
                            <button onClick={() => navigate('/dashboard')} className="mt-4 text-primary font-black text-xs uppercase tracking-widest hover:underline">Return to Command Center</button>
                        </div>
                    ) : type === 'ai-assistant' ? (
